    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#1e3a52]/50 rounded-2xl border border-white/10 p-5">
        ...
        <div className="space-y-2 text-sm mb-4">
          <div className="flex justify-between">
            <span className="text-white/50">Сесия</span>
            <span className="text-white font-medium">{sessionName}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-white/50">Деца</span>
          ...
          {totalAmount && (
            <div className="flex justify-between border-t border-white/10 pt-2 mt-2">
              ...
              <span className="text-[#f5a623] font-bold">{formatPrice(totalAmount)}</span>
            </div>
          )}
        </div>
        <div className="p-3 rounded-lg bg-[#f5a623]/10 border border-[#f5a623]/20 text-xs text-[#f5a623]">
      ...
      <div className="space-y-3">
        ...
        <button
          ...
          disabled={loading}
          className="w-full flex items-center gap-4 p-5 rounded-2xl bg-[#1e3a52]/50 border border-white/10 hover:border-[#f5a623]/40 hover:bg-[#1e3a52]/80 transition-all group text-left disabled:opacity-50"
        >
          <div className="w-12 h-12 rounded-xl bg-[#f5a623]/15 flex items-center justify-center text-2xl shrink-0">
            💳
          </div>
      ...
      <button
        onClick={onBack}
        className="w-full py-3 bg-white/10 hover:bg-white/15 text-white/60 font-medium rounded-xl text-sm transition-all"
      >
        ← Назад
      </button>

      {loading && (
        <p className="text-center text-white/40 text-xs animate-pulse">Пренасочване...</p>
      )}
    </div>
  );
}
