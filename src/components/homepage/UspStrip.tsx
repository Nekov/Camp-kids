export default function UspStrip() {
  ...
  return (
    <section className="bg-[#1e3a52]/40 border-y border-white/10 py-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6">
          {usps.map((usp) => (
            <div key={usp.title} className="flex flex-col items-center text-center gap-2">
              ...
              <p className="text-white font-semibold text-sm leading-tight">{usp.title}</p>
              <p className="text-white/50 text-xs leading-relaxed">{usp.desc}</p>
            </div>
          ))}
        </div>
      </div>
